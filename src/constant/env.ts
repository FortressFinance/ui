...
const network_declared = [
  {
    name: "Mainnet fork",
    supported: process.env.NEXT_PUBLIC_MAINNETFORK_SUPPORTED ?? "false",
  },
  {
    name: "Arbitrum fork",
    supported: process.env.NEXT_PUBLIC_ARBITRUMFORK_SUPPORTED ?? "false",
  },
  {
    name: "Arbitrum One",
    supported: process.env.NEXT_PUBLIC_ARBITRUM_SUPPORTED ?? "false",
  },
]
...
network_declared.map((networkVar) => {
  ...
  const network =
    networkVar.name.toLocaleUpperCase() === "MAINNET FORK"
      ? mainnetFork
      : networkVar.name.toLocaleUpperCase() === "ARBITRUM FORK"
      ? arbitrumFork
      : arbitrum
  let provider: ChainProviderFn | undefined = undefined
  if (
    ["MAINNET FORK", "ARBITRUM FORK"].includes(
      networkVar.name.toLocaleUpperCase()
    )
  ) {
...
export const CHAIN_ID =
  ENABLE_CHAINS.length > 0 ? ENABLE_CHAINS[0].id : undefined
