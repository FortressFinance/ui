...
export const ConcentratorVaultDepositForm: FC<ConcentratorVaultProps> = (
  ...
  const inputIsLp = inputTokenAddress === props.primaryAsset
  ...
  const allowance = useContractRead({
    ...
    args: [userAddress ?? "0x", concentrator?.data?.ybTokenAddress],
  ...
  const prepareApprove = usePrepareContractWrite({
    ...
    args: [concentrator?.data?.ybTokenAddress, ethers.constants.MaxUint256],
  ...
  const { data: concentratorId } = useConcentratorId(props)
  const isCurve = useIsConcentratorCurveVault(props.targetAsset)

  const previewDeposit = useConcentratorPreviewDeposit({
    chainId,
    targetAssetId,
    concentratorId,
  ...
  const vaultContract = useVaultContract(concentrator?.data?.ybTokenAddress)
  ...
  return (
    <div className="p-3 md:rounded-md md:bg-pink-100/10 lg:p-4">
      ...
      <ConfirmTransactionModal
        ...
        outputTokenAddress={concentrator?.data?.ybTokenAddress}
