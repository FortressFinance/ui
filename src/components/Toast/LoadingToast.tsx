import { FC, PropsWithChildren } from "react"

import Spinner from "@/components/Spinner"
import { Toast, ToastComponentProps } from "@/components/Toast"

type LoadingToastComponentProps = ToastComponentProps & {
  text?: string
}

export const LoadingToast: FC<
  PropsWithChildren<LoadingToastComponentProps>
> = ({ text = "Loading...", isVisible }) => {
  return (
    <Toast
      isVisible={isVisible}
      className="flex items-center bg-sky-500 text-white "
    >
      <div className="text-blue-500 bg-blue-100 dark:bg-blue-800 dark:text-blue-200 inline-flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg">
        <Spinner className="white col-span-full row-span-full h-5 w-5" />
      </div>
      <div className="text-md ml-3 font-normal">{text}</div>
    </Toast>
  )
}
