...
type MultiLayerTokenLogoProps = {
  tokens: `0x${string}`[] | readonly `0x${string}`[] | undefined,
  vaultType: VaultType,
  isLpToken: boolean,
  className: string,
  size: number,
...
const BaseLogo: FC<BaseLogoProps> = ({ vaultType, isLpToken, className }) => {
  switch (vaultType) {
    case "curve":
      return (isLpToken ? <Image src={CurveLogo} className={className} priority alt="" /> : <Image src={FortressCurveLogo} className={className} priority alt="" />)
    case "balancer":
      return (!isLpToken ? <Image src={BalancerLogo} className={className} priority alt="" /> : <Image src={FortressBalancerLogo} className={className} priority alt="" />)
    default:
      return null
  }
}

const MultiLayerTokenLogo: FC<MultiLayerTokenLogoProps> = ({ tokens, vaultType, isLpToken, className, size }) => {

  const getSubLogosPositions = (numberOfCurrencyIcons: number | undefined, rx: number, ry: number) => {
  ...
  return (
    <span id="mull" className="w-8 h-8 relative" style={{ fontSize: 12 }} >
      <BaseLogo vaultType={vaultType} isLpToken={isLpToken} className={className}></BaseLogo>
      {
        tokens && tokens.map((token, index) => {
          const top: string = (size / 1.5 - 1 + positions[index].x) + 'px'
          const left: string = (size / 1.5 + positions[index].y) + 'px'

          return (<AssetLogo key={`sublogo-${index}`} className="absolute" name="token" tokenAddress={token} style={{ top: top, left: left, width: size / 2, height: size / 2 }} />)
        })
      }
    </span>
  )
